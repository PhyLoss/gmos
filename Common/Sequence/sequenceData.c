...
Sequence *revcomp(Sequence *seq){
  ...
  char c;
  Sequence *newSeq;
  newSeq = (Sequence *)/*e*/malloc(sizeof(Sequence));

/*   n = strlen(seq->seq); */
  n = seq->len;
  newSeq->seq = (char *) /*e*/malloc((size_t)(n+1)*sizeof(char));
...
#endif
  Sequence *s;
	s = (Sequence *)/*e*/malloc(sizeof(Sequence));
 // s->freqTab = (Int64 *)/*e*/malloc(DICSIZE * sizeof(Int64));
	//for(i = 0; i < DICSIZE; i++) {
 //   s->freqTab[i] = 0;
	//}
	s->freqTab = NULL; /* matrix: numberOfSequences x DICSIZE*/

	s->borders = (Int64 *)/*e*/malloc(sizeof(Int64));
  s->headers = (char **)/*e*/malloc(sizeof(char *));
  s->id = (char *)/*e*/malloc(6 * sizeof(char)); // 6 for "strId"
  s->id[0] = '\0';
  strcat(s->id, "strId");
  s->len = 0;
  s->numSeq = 0;
 ...
 */
Sequence *readFasta(int fd, char ambiguous_char) {
  Sequence *s;
  char buf[BUFSIZ], x;
  int headerOpen;
  int headerLen;
  ...
  headerLen = 0;
    ...
    if (s->len + c + 1 > maxLen) {
      if (maxLen >= BUFSIZ) {
				...
				maxLen = BUFSIZ;
				s->seq = (char *)/*e*/malloc((maxLen + 2)*sizeof(char));
      }
    }
    ...
    for (i = 0; i < c; i++) {			
      ...
      if(buf[i] == '>'){ 
				...
				s->headers[s->numSeq] = (char *)/*e*/malloc((BUFSIZ+1)*sizeof(char));
				headerLen = 0;
				s->numSeq++;

			  s->freqTab = (Int64 **)erealloc(s->freqTab, (size_t)(s->numSeq) * sizeof(Int64 *));
				s->freqTab[s->numSeq - 1] = (Int64 *)/*e*/malloc((size_t)(DICSIZE) * sizeof(Int64));
				for (j = 0; j < DICSIZE; j++) {
      ...
      if(headerOpen){
        ...
        x = toupper(buf[i]);
        if (x != 'A' && x != 'C' && x != 'G' && x != 'T') {
          x = ambiguous_char; // mask ambiguous characters
        }
				//s->seq[s->len] = buf[i];
				s->seq[s->len] = x;
				//++ s->freqTab[s->numSeq - 1][(int)buf[i]];
				++ (s->freqTab[s->numSeq - 1][(int)x]);
				++ (s->len);
			} // end if
...
static Sequence *getArraySElement(Sequence *s, Int64 i, Int64 min) {
	...
	elem = (Sequence *)/*e*/malloc(sizeof(Sequence));
	elem->len = len = s->borders[i] - min + 1; // length of the i-th strain including its right border

	/* copy char-s from i-th strain (+ 1 for '\0') */
	elem->seq = (char *)/*e*/malloc((size_t)(len + 1) * sizeof(char)); 
	strncpy(elem->seq, s->seq + min, (size_t)len); /* s should be adjusted for i > 0!!*/
	elem->seq[len] = '\0';
	elem->id = (char *)/*e*/malloc((strlen(s->id) + 1)* sizeof(char)); /* +1 for '\0'*/
	strcpy(elem->id, s->id);
	elem->numSeq = 1;
	elem->borders = (Int64 *)/*e*/malloc(sizeof(Int64));
	elem->borders[0] = len - 1;
	
	elem->headers = (char **)/*e*/malloc(sizeof(char *)); /* one header, since only one strain */
	elem->headers[0] = (char *)/*e*/malloc((strlen(s->headers[i]) + 1) * sizeof(char)); /* +1 for '\0'*/
	strcpy(elem->headers[0], s->headers[i]);
	elem->numSbjctNuc = elem->numNuc = len - 1;
	/* freqTab is a matrix: 1 x DICSIZE */
	elem->freqTab = (Int64 **)/*e*/malloc(sizeof(Int64 *));
	elem->freqTab[0] = (Int64 *)/*e*/malloc(DICSIZE * sizeof(Int64));
	for (k = 0; k < DICSIZE; k++) {
...
Sequence **getArrayOfSeq(Sequence *s) {
	...
	arrayS = (Sequence **)/*e*/malloc((size_t)(s->numSeq)* sizeof(Sequence *));
...
void prepareSeq(Sequence *sequence){
  ...
  rstrand->headers = (char **)/*e*/malloc(sizeof(char *));
  rstrand->headers[0] = (char *)/*e*/malloc(sizeof(char));
  rstrand->borders = (Int64 *)/*e*/malloc(sizeof(Int64));
...
Sequence *catSeq(Sequence *seq1, Sequence *seq2, int flag){
  ...
  /* this part doesn't depend on the flag */
	cat = (Sequence *)/*e*/malloc(sizeof(Sequence));
  cat->seq = (char *)/*e*/malloc((strlen(seq1->seq) + strlen(seq2->seq) + 1)*sizeof(char));
  cat->seq[0] = '\0';
  ...
  cat->id = (char *)/*e*/malloc(6 * sizeof(char));
  cat->id[0] = '\0';
  strcat(cat->id, "strId");
  n = seq1->numSeq + seq2->numSeq;
  cat->numSeq = n;
  //cat->borders = (int *)/*e*/malloc(2*n*sizeof(int));
  cat->borders = (Int64 *)/*e*/malloc(2 * (size_t)n * sizeof(Int64));
  cat->headers = (char **)/*e*/malloc((size_t)n * sizeof(char *));
  
	/* frequency table */
	//for(i = 0; i < DICSIZE; i++) {
	//	cat->freqTab[i] = seq1->freqTab[i] + seq2->freqTab[i];
	//}
	cat->freqTab = (Int64 **)/*e*/malloc((size_t)n * sizeof(Int64 *));
	for (i = 0; i < seq1->numSeq; i++) {
		cat->freqTab[i] = (Int64 *)/*e*/malloc(DICSIZE * sizeof(Int64));
		for (k = 0; k < DICSIZE; k++) {
			cat->freqTab[i][k] = seq1->freqTab[i][k];
		}
	}
	for (i = 0; i < seq2->numSeq; i++) {
		cat->freqTab[seq1->numSeq + i] = (Int64 *)/*e*/malloc(DICSIZE * sizeof(Int64));
		for (k = 0; k < DICSIZE; k++) {
  ...
  for(i = 0; i < seq1->numSeq; i++){
    cat->headers[i] = (char *)/*e*/malloc((strlen(seq1->headers[i]) + 1) * sizeof(char));
    cat->headers[i] = strcpy(cat->headers[i], seq1->headers[i]);
  }
  j = i;
  for(i = 0; i < seq2->numSeq; i++){
    cat->headers[j] = (char *)/*e*/malloc((strlen(seq2->headers[i]) + 1) * sizeof(char));
...
Sequence *cloneSeq(Sequence *ori){
	...
	clone = (Sequence *)/*e*/malloc(sizeof(Sequence));
	clone->seq = (char *)/*e*/malloc((ori->len + 1) * sizeof(char));
	clone->seq = strncpy(clone->seq, ori->seq, ori->len);
	
	clone->id = (char *)/*e*/malloc((strlen(ori->id) + 1) * sizeof(char));
	clone->id = strncpy(clone->id, ori->id, 6);
	clone->numSeq = ori->numSeq;
	clone->numQuery = ori->numQuery;
	
	clone->borders = (Int64 *)/*e*/malloc(ori->numSeq * sizeof(Int64));	
	for (i = 0; i < ori->numSeq; i++) {
		clone->borders[i] = ori->borders[i];
	}
	
	clone->headers = (char **)/*e*/malloc(ori->numSeq * sizeof(char *));
	for (i = 0; i < ori->numSeq; i++) {
		clone->headers[i] = (char *)/*e*/malloc((strlen(ori->headers[i]) + 1) * sizeof(char));
		clone->headers[i] = strcpy(clone->headers[i], ori->headers[i]);
	}
	
	clone->len = ori->len;
	clone->freqTab = (Int64 **)/*e*/malloc(clone->numSeq * sizeof(Int64 *));
	for (i = 0; i < clone->numSeq; i++) {
		clone->freqTab[i] = (Int64 *)/*e*/malloc(DICSIZE * sizeof(Int64));
		for (j = 0; j < DICSIZE; j++) {
