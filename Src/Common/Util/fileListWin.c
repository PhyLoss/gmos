...
#define MAXFILE 100
	...
	char **listFilesWin(char *dir, int *n) {
		...
		fileList = (char **)/*e*/malloc(sizeof(char *) * i);
		...
		if (hFind == INVALID_HANDLE_VALUE) {
			...
			do {
				...
				if(!(FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) && strstr(fileName, ".fasta")) { /* FILE_ATTRIBUTE_DIRECTORY = file is a dir*/
					...
					//fileList[*n] = (char *)/*e*/malloc(strlen(dir) + 1 + strlen((const char *)FindFileData.cFileName) + 1); /* + 2 for \ and \0*/
					fileList[*n] = (char *)/*e*/malloc(strlen(dir) + 1 + strlen(fileName) + 1); /* + 2 for \ and \0*/
